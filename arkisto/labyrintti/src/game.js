...
(function () {
  ...
  function draw() {
    ...
    var cssH = parseFloat(canvas.style.height) || (cssW * v.height / v.width);
    var fitScale = cssW / v.width;

    // Pienellä ruudulla zoomataan sisään ja kamera seuraa kuulaa,
    // jottei pallo jää liian pieneksi. Isolla näytöllä näkyy koko kenttä.
    var minBallPx = 17;
    var scale = (cssW < 430 && !document.fullscreenElement)
      ? Math.max(fitScale, minBallPx / (2 * v.ballR))
      : fitScale;
    var viewW = cssW / scale, viewH = cssH / scale;
    var camX = 0, camY = 0;
    if (scale > fitScale + 1e-4) {
      camX = Math.min(Math.max(v.x - viewW / 2, 0), Math.max(0, v.width - viewW));
      camY = Math.min(Math.max(v.y - viewH / 2, 0), Math.max(0, v.height - viewH));
    }
