/* Diagnostiikka-ajuri. Sama botti kuin bot.js, mutta tulostaa kuolemaa edeltävät
   ruudut ja halutessa lentoradan x-väliltä. Tämä paljasti käytännössä joka juurisyyn.
     node tools/trace.js 7                 kuolinruudut K8:sta
     node tools/trace.js 8 --arc 3000 5000 lentorata x-väliltä */
...
function botRun(lvl,maxSec){
  ...
  for(f=0; f<maxSec*60; f++){
    if(game.state==='dead'){
      if(globalThis.__rb && !globalThis.__dumped){
        console.log('KUOLEMA @ '+((game.bike.body.x*34)|0)+' — viimeiset '+globalThis.__rb.length+' ruutua:');
        globalThis.__rb.forEach(r=>console.log('  '+r));
    ...
    { const R=(globalThis.__rb=globalThis.__rb||[]);
      R.push([(b.body.x*34)|0,(b.body.y*-34)|0,'v='+v.toFixed(1),'vy='+b.body.vy.toFixed(1),
              (b.w2c?'T':'-')+(b.w4c?'E':'-'),'a='+a.toFixed(0),'av='+av.toFixed(1),'d'+b.dir,
              inp.gas?'K':'.',inp.brake?'J':'.',(inp.voltL?'VL':'')+(inp.voltR?'VR':'')].join(' '));
      if(R.length>24) R.shift();
      const XR=globalThis.__xr;
      if(XR && b.body.x*34>XR[0] && b.body.x*34<XR[1] && af%2===0) console.log('  |',R[R.length-1]);
    }
...
{ const i=argv.indexOf('--arc');
  if(i>=0){ globalThis.__xr=[+argv[i+1],+argv[i+2]]; argv.splice(i,3); } }
