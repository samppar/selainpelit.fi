...
function renderTable(trick, ledSuit, winnerSeat) {
  ...
  const cells = order.map((s) => {
    ...
    const rami = currentRamaaja === s ? " ramaaja" : "";
    const badge = currentRamaaja === s ? '<span class="ramibadge">RAMI</span>' : "";
    return '<div class="seatcell' + me + ally + win + rami + '"><div class="seatname">' +
      seatNameShort(s) + badge + "</div>" +
...
let currentRamaaja = null; // kuka ramasi tässä jaossa (null = nolo)
...
// Näytä pelimuoto ja kuka ramasi — selkeästi nimellä.
function renderGameType(gameType, ramaaja, humanSeat) {
  currentRamaaja = (gameType === "rami" && ramaaja !== null) ? ramaaja : null;
  if (gameType !== "rami" || ramaaja === null) {
    el("gametype").innerHTML =
      '<div class="gtbanner nolo">' +
      '<span class="gt-nolo">NOLO</span>' +
      '<span class="gtmain">Kaikki nolasivat — vältä kasoja</span>' +
      "</div>";
    return;
  }
  const who = seatName(ramaaja);
  const side =
    ramaaja === humanSeat ? "sinun joukkueesi kerää" :
    teamOf(ramaaja) === teamOf(humanSeat) ? "joukkueesi kerää" : "vastustajat keräävät";
  el("gametype").innerHTML =
    '<div class="gtbanner rami">' +
    '<span class="gt-rami">RAMI</span>' +
    '<span class="gtmain"><b>' + who + "</b> ramasi</span>" +
    '<span class="gtsub">' + side + " — tavoite yli 6 kasaa</span>" +
    "</div>";
}
...
async function playInteractiveDeal(humanSeat, level, rng, mstate) {
  ...
  if (ramaaja !== null) {
    showToast(seatName(ramaaja) + " ramasi", teamOf(ramaaja) === teamOf(humanSeat) ? "good" : "bad");
    renderTable([], null);
  } else {
    showToast("Nolo — kaikki nolasivat", "good");
  }
...
function askHumanSooli(hand, mstate, myTeam, oppTeam) {
  return new Promise((resolve) => {
    ...
    const who = currentRamaaja != null ? seatName(currentRamaaja) : "Vastustaja";
    el("handLabel").innerHTML =
      "<b>" + who + "</b> ramasi. Haluatko pelata <b>soolon</b> — yksin heitä vastaan? " +
...
.gtbanner.rami { border-color: rgba(209,39,75,.45); background: rgba(209,39,75,.12); }
.gtbanner.nolo { border-color: var(--line); }
.gtmain { font-size: 1.05rem; font-weight: 700; }
.gtbanner.rami .gtmain b { color: #ffb3c0; font-size: 1.15rem; }
.gtsub { width: 100%; font-size: .82rem; font-weight: 500; color: var(--muted); }
.tricks {
...
.gt-rami, .gt-nolo {
  font-weight: 800; letter-spacing: .05em; padding: .15rem .55rem;
  border-radius: 999px; font-size: .82rem;
}
.gt-rami { background: rgba(209,39,75,.28); color: #ff6b8a; }
...
.seatcell.ramaaja { filter: drop-shadow(0 0 14px rgba(209,39,75,.55)); }
.seatcell.ramaaja .seatname {
  background: rgba(209,39,75,.85); color: #fff;
}
.ramibadge {
  display: inline-block; margin-left: .35rem; padding: .05rem .35rem;
  border-radius: 999px; background: #fff; color: var(--red);
  font-size: .58rem; font-weight: 800; letter-spacing: .06em; vertical-align: middle;
}
.empty {
